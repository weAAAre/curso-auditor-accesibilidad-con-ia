# Fuentes primarias

Las skills del curso deben priorizar estas fuentes y enlazarlas cuando una
decisión dependa de ellas.

- [WCAG Evaluation Methodology 2.0](https://www.w3.org/TR/wcag-em-2/)
- [WCAG 2.2](https://www.w3.org/TR/WCAG22/)
- [ACT Rules Format 1.1](https://www.w3.org/TR/act-rules-format/)
- [Axe-core](https://github.com/dequelabs/axe-core)
- [API de axe-core](https://github.com/dequelabs/axe-core/blob/develop/doc/API.md)
- [Agent Browser](https://agent-browser.dev/)
- [Comandos y auditoría de accesibilidad de Agent Browser](https://agent-browser.dev/commands)
- [Skill oficial de Agent Browser](https://github.com/vercel-labs/agent-browser/tree/main/skills/agent-browser)
- [Agent Skills Specification](https://agentskills.io/specification)
- [Skills de Codex](https://learn.chatgpt.com/docs/build-skills)
- [Skills de Claude Code](https://code.claude.com/docs/en/slash-commands)

Fecha de revisión de esta lista: 20 de agosto de 2026.
